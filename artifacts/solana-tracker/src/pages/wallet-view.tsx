      ...
      <div className="p-8 flex flex-col items-center justify-center h-full">
        <div className="p-6 bg-destructive/10 border border-destructive/20 rounded-2xl text-center max-w-md">
          ...
          <p className="text-muted-foreground">
            The wallet address might be invalid or there was an error connecting
            to the Solana RPC.
          </p>
        </div>
      </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <Badge
                variant="outline"
                className="bg-white/5 hover:bg-white/10 text-xs font-medium uppercase tracking-wider py-1 border-white/10"
              >
                Tracking Active
              </Badge>
              <div className="flex items-center gap-1.5 text-xs font-medium">
                {wsStatus === "connected" ? (
                  <>
                    ...
                    <span className="text-primary">Live (WS)</span>
                  </>
                ) : (
                  <>
          ...
          <div className="flex gap-4">
            ...
            <div className="bg-black/20 rounded-xl p-4 border border-white/5 min-w-[120px]">
              ...
              <div className="text-sm font-medium mt-2">
                {wallet
                  ? formatDistanceToNow(new Date(wallet.createdAt), {
                      addSuffix: true,
                    })
                  : "Just now"}
              </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-muted-foreground uppercase tracking-wider bg-black/20 sticky top-0 z-10 backdrop-blur-md">
              <tr>
                ...
                <th className="px-6 py-4 font-medium border-b border-white/5 text-center">
                  Status
                </th>
              </tr>
            </thead>
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                            <button
                              onClick={() =>
                                handleCopy(tx.signature, "Signature")
                              }
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-center">
                        {tx.status.toLowerCase() === "success" ? (
                          <Badge
                            variant="outline"
                            className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 font-medium"
                          >
                            Success
                          </Badge>
                        ) : (
